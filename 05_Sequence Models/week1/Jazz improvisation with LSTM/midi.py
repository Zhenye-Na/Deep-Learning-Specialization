...
# Uses FFmpeg to convert WAV files to MP3. Requires ffmpeg.exe in the script folder or PATH
...
try:
    ...
    for song in matches:
        ...
        stream = audio.open(format=format, channels=channels, rate=sample_rate,
                            input=True, input_device_index=input_device, frames_per_buffer=buffer)
        ...
        if do_ffmpeg_convert:
            os.system('ffmpeg -i ' + new_file + ' -y -f mp3 -ab ' + str(mp3_bitrate) + 'k -ac ' +
                      str(channels) + ' -ar ' + str(sample_rate) + ' -vn ' + file_name + '.mp3')
